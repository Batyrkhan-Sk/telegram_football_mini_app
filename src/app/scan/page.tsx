...
import { QrCode, CheckCircle, XCircle, Clock, ChevronRight, Scan } from 'lucide-react'
...
const REWARD_LABELS: Record<string, { icon: string; label: string; color: string }> = {
  ...
  AR_COOLDOWN_BYPASS: { icon: '🎯', label: 'AR Cooldown Removed', color: 'text-green-400' },
  ...
  const { mutate: redeem, isPending } = useMutation({
    mutationFn: () =>
      fetch('/api/scan', {
        ...
        body: JSON.stringify({ telegramId: user?.telegramId, code: code.trim().toUpperCase() }),
      }).then((r) => r.json()),
    onSuccess: (res) => {
      if (res.error) {
        setError(res.error)
  ...
  return (
    <div className="flex flex-col min-h-screen pb-24">
      <div className="px-4 pt-4 space-y-5">
        <SectionHeader title="Scan QR" subtitle="Redeem SNICKERS promo codes" />

        {/* Scan illustration */}
        <div className="snickers-strip h-1 rounded-full" />
        <div className="relative bg-gradient-to-b from-red-950/60 to-surface-2 border border-red-800/30 rounded-3xl p-6 flex flex-col items-center gap-4 overflow-hidden">
          ...
          <div className="relative z-10 text-center">
            <h3 className="font-display font-900 text-xl uppercase text-red-300">SNICKERS QR Code</h3>
            <p className="text-xs text-red-400/70 mt-1">Find codes on SNICKERS packaging in Kazakhstan</p>
          </div>
        </div>
        ...
        {/* Code input */}
        <div>
          <label className="text-[11px] font-display uppercase text-gray-400 block mb-1.5">Enter Promo Code</label>
        ...
        <div className="bg-surface-2 border border-white/6 rounded-2xl p-3">
          <p className="text-[10px] font-display uppercase text-gray-500 mb-2">Demo Codes (for testing):</p>
